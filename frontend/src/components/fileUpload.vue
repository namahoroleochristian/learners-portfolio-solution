<template>
  <div class="m-25" style="background-image: url('https://pfst.cf2.poecdn.net/base/image/0935f79852dea24c2fd6768a80c491e649f17294fd8005edbbef6672a8a536ee?w=1024&h=768&pmaid=289842107');">
    <form @submit.prevent="handleFileUpload" class="min-h-120 border border-gray-200 shadow-2xl rounded-4xl">
      ...
      <div v-if="uploadStatus == 'Please select a file first.'" class="flex justify-center text-center text-xl h-16 bg-red-100 p-4 w-5/6 text-red-800 m-25 mt-10">
        <p class="align-center">** {{ uploadStatus }}</p>
      </div>
      <div v-if="uploadStatus == 'Uploading...'" class="flex justify-center text-center text-xl h-16 bg-blue-100 p-4 w-5/6 text-blue-800 m-25 mt-10">
        <p class="align-center">** {{ uploadStatus }}</p>
      </div>
      <div v-if="uploadStatus == 'File uploaded successfully' " class="flex justify-center text-center text-xl h-16 bg-green-200 p-4 w-5/6 text-green-800 m-25 mt-10">
        <p class="align-center">** {{ uploadStatus }}</p>
      </div>
    </form>
  </div>
...
export default {
  ...
  // Display a readable error message to the user
  this.uploadStatus = error.response && error.response.data && error.response.data.message
